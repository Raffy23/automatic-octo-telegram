...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == automatic-octo-telegram
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "automatic-octo-telegram" {
	...
	// Get SLI File from automatic-octo-telegram subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=automatic-octo-telegram/sli.yaml
	sliFile := "automatic-octo-telegram/sli.yaml"
